...
setwd("~/Documents/Work/data.nosync/GEO/")
id <- 'GSE172365'
dir.create(id)
setwd(file.path(getwd(), id))
...
# download data
getGEOSuppFiles(id, filter_regex = 'GSE172365_matrix.signal.intensities.txt.gz', fetch_files = T, makeDirectory = F)
gunzip("GSE172365_matrix.signal.intensities.txt.gz")
list.files()

unlink("GSE108123/", recursive = T)

# Preprocess raw data from signal intensities
readLines("GSE172365_matrix.signal.intensities.txt", n = 1)
Mset <- minfi::readGEORawFile(filename = "GSE108123_matrix_signal_intensities.txt",
                              sep = "\t",
                              Uname = "_Methylated.signal",
                              Mname = "ethylated Signal",
